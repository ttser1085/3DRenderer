...
public:
	Vertex operator()(const Vec3& brc, const Mesh& mesh) {
		return Vertex{lalg::linearCombination<Vec4>(brc, mesh.vertices[0].pos,
													mesh.vertices[1].pos,
													mesh.vertices[2].pos),
					  lalg::linearCombination<Color3f>(
						  brc, mesh.vertices[0].color, mesh.vertices[1].color,
...
public:
	Vertex operator()(const Vec3& brc, const Mesh& mesh) {
		Float z = lalg::linearCombination<Float>(brc, mesh.vertices[0].pos(2),
												 mesh.vertices[1].pos(2),
												 mesh.vertices[2].pos(2));

		return Vertex{Vec4{0.0f, 0.0f, z, 1.0f}, mesh.vertices[0].color};
	}
};
